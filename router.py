...
from admin.auth_user_setting_resource import AuthUserSetting, AuthUserSettingByPrefix, AuthUserSettingPost
from admin.auth_role_resource import AuthRoles, AuthRole, AuthRolePost
from admin.adm_setting_resource import AdmSettings, AdmSettingByPrefix, AdmSetting, AdmSettingPost
...
from resources.client_address_resource import ClientAddress, ClientAddressCurrent, ClientAddressByClient
...
from resources.clientisrael_resource import ClientIsraels, ClientIsraelsByClient, ClientIsrael
from resources.clientloan_resource import ClientLoans, ClientLoan, ClientLoanByClient
from resources.credit_report_resource import CreditReports, CreditReport, CreditReportByClient
from resources.credit_line_history_resource import CreditLineHistories, CreditLineHistory, CreditLineHistoryByClient
...
from resources.cccompany_resource import CcCompanies, CcCompanies, CcCompany, CcCompanyPost
    ...
    def apply_jwt(response):
        if not (request.method == "OPTIONS" or response is None):
            if request.path.__eq__("/onboard/login") and request.method == "POST":
                username = request.json['username'] if 'username' in request.json else '__unknown__'
...
def sundry_routes( app):
    ...
    def read_main():
        ...
        return cs.json_rc_msg( 1, 'Welcome to CBMS', {'version': os.getenv('API_VERSION'), "time":  dt })
    ...
    def ping():
        ...
        dt = datetime.datetime.now().strftime( "%b %d %Y %H:%M:%S")
        return cs.json_rc_msg( 1, "Ping", { "hostname": socket.gethostname(), "time":  dt, "API_VERSION": os.getenv( 'API_VERSION'), "Connect": connect_string})

    @app.route('/pingos', methods=['GET'])
    def pingos():
        exclude_keys = ['JWT_SECRET_KEY', 'ENCRYPT_KEY', 'SQLCONNECTION_STRING', 'CONNECTION_STRING' ]
        return cs.json_rc_msg( 1, "PingOS", json.dumps([ { x: os.getenv(x)} for x in os.environ.keys() if x not in exclude_keys ]))
